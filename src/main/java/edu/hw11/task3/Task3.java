...
public final class Task3 {
    ...
    private static class FibByteCodeAppender implements ByteCodeAppender {
        @Override
        public @NotNull Size apply(
            ...
            // if (n <= 0)
            mv.visitVarInsn(Opcodes.ILOAD, 0);  // Загрузка аргумента n в стэк
            mv.visitJumpInsn(Opcodes.IFLE, lessOrEqualsZero);  // Если n <= 0 переход к метке lessOrEqualsZero

            // if (n == 1)
            mv.visitInsn(Opcodes.ICONST_1); // Загрузка константы 1 в стэк
            mv.visitVarInsn(Opcodes.ILOAD, 0);  // Загрузка аргумента n в стэк
