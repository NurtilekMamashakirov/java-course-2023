...
public class Tasks {
    ...
    //task2
    public List<Animal> reverseSortByWeightAndChooseK(List<Animal> animals, Integer k) {
    ...
    public Animal longestName(List<Animal> animals) {
        animals =
            animals.stream().sorted((Animal o1, Animal o2) -> Integer.compare(o2.name().length(), o1.name().length()))
                .limit(1)
                .toList();
    ...
    //task6
    public Map<Animal.Type, Animal> heaviestAnimalOfTypes(List<Animal> animals) {
    ...
    //taks8
    public Optional<Animal> heaviestAndBelowK(List<Animal> animals, Integer k) {
    ...
    public List<Animal> animalsWithAgeNotEqualsPaws(List<Animal> animals) {
        animals = animals.stream().filter(animal -> {
            if (animal.type().equals(Animal.Type.DOG) || animal.type().equals(Animal.Type.CAT)) {
                return animal.age() != 4;
            } else if (animal.type().equals(Animal.Type.BIRD)) {
                return animal.age() != 2;
    ...
    //task11
    public List<Animal> canBiteAndHigher100(List<Animal> animals) {
    ...
    //taks15
    public Map<Animal.Type, Integer> summarizeWeightWithCondition(List<Animal> animals, Integer k, Integer i) {
        Map<Animal.Type, Integer> sumWeight = new HashMap<>();
        animals.stream().filter(animal -> animal.age() >= k && animal.age() <= i).toList()
            .forEach(animal -> {
    ...
    //task17
    public Boolean spiderBitesMoreThanDog(List<Animal> animals) {
    ...
    //task19
    Map<String, ValidationError> findErrors(List<Animal> animals) {
    ...
    //task20
    Map<String, String> findErrorsWithMessage(List<Animal> animals) {
        Map<String, String> errorsOfAnimals = new HashMap<>();
        animals.stream().forEach(a -> {
            ValidationError validationError = new ValidationError();
            validationError.checkAllErrors(a);
            String message = validationError.getNameError().getMessage() + " " + validationError.getAgeError().getMessage() + " "
                + validationError.getHeightError().getMessage() + " " + validationError.getWeightError().getMessage();
